...
def generate_fix_report(stats: dict) -> str:
    ...
    if stats['fix_commands']:
        ...
        for i, cmd in enumerate(stats['fix_commands'], 1):
            if cmd['type'] == 'rewrite':
                report.append(
                    f"# {i}. {Path(cmd['transcript']).parent.name}"
                )
                report.append(
...
def auto_fix_missing_rewritten(archive_root: str = 'content_archive', days: int = 30):
    ...
    for cmd in stats['fix_commands']:
        ...
        try:
            result = rewrite_service.rewrite_content(
                cmd['transcript'],
                cmd['metadata'],
                cmd['output']
            )
...
def main():
    parser = argparse.ArgumentParser(
        description='内容完整性验证工具'
    )
    parser.add_argument(
        '--days', '-d',
        type=int,
        default=30,
        help='检查最近 N 天的内容（默认 30，0 表示全部）'
    )
    parser.add_argument(
        '--only-invalid',
        action='store_true',
    ...
    parser.add_argument(
        ...
        help='内容存档根目录（默认 content_archive）'
    )
