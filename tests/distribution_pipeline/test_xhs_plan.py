...
def test_build_xhs_card_plan_growth_depth_compresses_without_losing_body_route():
    ...
    assert [card["insight_index"] for card in plan if card["type"] == "single-insight"] == [1, 2, 3, 5, 7, 8, 10]
    assert plan[-2]["type"] == "philosophical-card"
...
def test_build_xhs_card_plan_growth_depth_uses_audience_hook():
    source = {"title": "How To Grow An Audience If You Have 0 Followers (It's Only 2 Habits)", "channel": "Dan Koe"}
    insights = [
