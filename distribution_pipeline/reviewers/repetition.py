...
def review_repetition(briefs: list[dict]) -> dict:
    ...
    for prev, current in zip(briefs, briefs[1:]):
        ...
        if prev.get("composition", {}).get("text_position") == current.get("composition", {}).get("text_position"):
            issues.append("duplicate_composition")

    positions = [brief.get("composition", {}).get("text_position") for brief in briefs if brief.get("composition")]
    if positions:
