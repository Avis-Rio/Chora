...
def run(
    ...
    if renderer == "guizang":
        ...
        if export_images:
            ...
            guizang_review = {}
            if platform in ("all", "xhs"):
                guizang_review["xhs"] = run_guizang_validator(package_dir / "xhs", mode=guizang_mode)
            if platform in ("all", "wechat"):
                guizang_review["wechat"] = run_guizang_validator(package_dir / "wechat", mode=guizang_mode)
        else:
            skipped_review = {
                "status": "skipped",
...
def main():
    ...
    parser.add_argument("--platform", default="all", choices=["all", "xhs", "wechat"])
    ...
    parser.add_argument(
        ...
        help="Guizang 图像资产处理模式：plan 仅写搜索计划，candidates 拉取候选，download 下载首选候选。",
    )
