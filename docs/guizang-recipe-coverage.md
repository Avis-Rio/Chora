...
当前已接入核心 Editorial 小红书流程、Swiss S01-S12 专属 renderer、Category cookbook 一期、图上叠字静态 QA、Swiss 数值抽取一期、`copy_slots` 文案分配层、重复文案/内部 scaffold label 静态 QA，以及 Swiss 尾卡双组 CTA，可稳定生成 3:4 卡组；但还不是完整 Guizang Skill。
...
- Swiss International：S01-S12 已有专属 renderer；planner 已能按图片、对比、风险、流程、清单、排名、数据等信号路由。
- 主题 token：Editorial 6 套、Swiss 4 套已可解析。
- 小红书 3:4：已落地核心链路。
- 微信公众号 21:9 + 1:1 封面对：已落地 Editorial cover pair，输出 21:9、1:1、pair preview 三个画板。
...
| `layout-recipes.md` | 基本落地 | Editorial 16/16 renderer 已接入；Swiss S01-S12 专属 renderer 已接入；category-aware sequence 已一期接入 | 强化 M06/M12、多截图、多地图等复杂资产条件 |
...
| `components.md` | 部分落地 | 字体、frame-img、issue-strip、ledger/marginalia、Swiss card-fill、device-browser、image-hero、kpi、h-bar、stacked-ledger、matrix 使用中 | 补 map block 与更细 screenshot treatment |
...
| `screenshot-treatment.md` | 未落地 | 当前无专属 screenshot renderer | 补 M06/S04/S08 截图容器与 object-fit 规则 |
| `map-component.md` | 未落地 | 当前无地图组件 | 旅行/空间内容需要时接入 Mapbox/OSM/static schematic |
...
| `production-workflow.md` | 部分落地 | 单文件多画板、Playwright PNG、SOURCES、WeChat 同 HTML 预览、静态 Guizang QA 已接入 | 补 render wait 与真实 thumbnail QA |
| `qa-checklist.md` | 部分落地 | 单元测试、validator、artifact contract、尺寸检查、R8/R10/R11 静态 QA 已有；Swiss fixture 已达到 `pass` | 自动化 thumbnail 像素检查、subject safety 目检辅助、style identity |
...
| S04 | Interface / Browser Mock | renderer 已接入 |
| S05 | Trap / Warning Rows | renderer 已接入，按风险/误区信号路由 |
| S06 | Pipeline / Architecture | renderer 已接入，按流程/系统信号路由 |
| S07 | Takeaway Ledger | renderer 已接入，用于收尾/结论 |
| S08 | Image Hero | renderer 已接入，有图优先路由 |
| S09 | KPI Tower | renderer 已接入，按指标/成本/增长信号路由 |
| S10 | H-Bar Chart | renderer 已接入，按排名/多点信号路由 |
| S11 | Stacked Ledger | renderer 已接入，按清单信号路由 |
| S12 | Matrix + Hero Stat | renderer 已接入，按多能力/多概念信号路由 |

Swiss 结论：

- Swiss 已从“通用占位”升级为 S01-S12 专属 renderer。
- `tests/fixtures/.../Token经济学` 的 Swiss 小红书真实导出已达到 validator `pass`，`5 clean · 0 fails · 0 warns`。
- 后续重点不再是 renderer 数量，而是 screenshot/map、多图语义、真实 thumbnail 像素 QA 和更强数值抽取。

## 已知设计债

- 短文本页不能默认落到 M03，否则在 3:4 画布上容易出现大片空白。
- 深色主题下，背景纹理会增强氛围，但也会抢小字号正文注意力；短正文更适合 M09/M11/M10。
- 图片 evidence 当前默认 4 张，仍需按内容语义优先级选择，而不是固定页码。
- Swiss 模板已有 S01-S12 专属 renderer；数据型 recipe 已先抽真实数字，无真数值时标记 proxy，后续需增强中文数字/复杂单位抽取。
...
4. 把 thumbnail readability 从静态 policy 检查升级为真实 360px 缩略图 QA。
